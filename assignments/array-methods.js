...
const mappedNames = runners.map((runners) => {
  let names = `${runners.first_name} ${runners.last_name}`;
fullName.push(names);
})
...
for(let i = 0; i < runners.length; i++) {
  let capsFirst = runners[i].first_name.toUpperCase();
  allCaps.push(capsFirst);
}
...
for(let i = 0; i < runners.length; i++) {
  let sizeSort = runners[i].shirt_size.filter('L');
  largeShirts.push(sizeSort);
}
